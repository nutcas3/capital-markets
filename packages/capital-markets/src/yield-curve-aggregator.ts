...
export class YieldCurveAggregator {
  ...
  constructor() {
    logger.info('YieldCurveAggregator initialized');
  }
   ...
   */
  async getYieldSources(symbol: string): Promise<YieldData[]> {
  ...
  async getBestYieldSource(
    ...
    const sources = await this.getYieldSources(symbol);
   ...
   */
  async getYieldCurve(symbol: string): Promise<YieldCurvePoint[]> {
  ...
  async calculateExpectedYield(
    ...
    amount: number,
    durationDays: number
  ): Promise<{ yieldAmount: number; apy: number }> {
    logger.info('Calculating expected yield', { symbol, amount, durationDays });
    
    // Get best yield source for the duration
    const bestSource = await this.getBestYieldSource(symbol, amount, durationDays);
   ...
   */
  async compareProviders(symbol: string): Promise<Record<string, number>> {
