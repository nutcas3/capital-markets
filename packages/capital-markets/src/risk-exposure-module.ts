...
export class RiskExposureModule {
  ...
  constructor(private readonly priceService: PriceService) {
    logger.info('RiskExposureModule initialized');
  }
   ...
   */
  async calculateRiskMetrics(portfolio: PortfolioAsset[]): Promise<RiskMetrics> {
   ...
   */
  async calculateSlippage(fromToken: string, toToken: string, amount: number): Promise<number> {
   ...
   */
  async calculateSlippageTolerance(fromToken: string, toToken: string, amount: number): Promise<number> {
    logger.info('Calculating slippage tolerance', { fromToken, toToken, amount });
    
    // Get estimated slippage
    const estimatedSlippage = await this.calculateSlippage(fromToken, toToken, amount);
